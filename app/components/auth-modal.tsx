...
export default function AuthModal({ onLoginSuccess }: AuthModalProps) {
  ...
  return (
    <div className="relative w-full max-w-5xl rounded-[36px] overflow-hidden shadow-[0_35px_100px_rgba(0,0,0,0.4)] border border-white/20 grid md:grid-cols-12 min-h-[630px] font-sans backdrop-blur-2xl bg-white/95 my-6 animate-fade-in transition-all duration-500">
      ...
      <div className="absolute -bottom-32 -right-32 w-96 h-96 bg-primary/30 rounded-full blur-3xl pointer-events-none animate-pulse"></div>

      {/* LEFT COLUMN: Visual Brand Showcase */}
      <div className="md:col-span-5 relative hidden md:flex flex-col justify-between p-10 overflow-hidden bg-gradient-to-br from-[#0c4e61] via-[#0e5a6f] to-[#062c37] text-white z-10">
        
        {/* Animated Background Resort Image */}
        <div className="absolute inset-0 z-0 overflow-hidden">
          <img
            src="https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1000&q=80"
            alt="Paradise Resort"
            className="w-full h-full object-cover scale-110 animate-subtle-zoom opacity-45 mix-blend-overlay transition-all duration-1000"
            onError={(e) => {
              (e.target as HTMLImageElement).style.display = "none";
            }}
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#08303d] via-[#0e5a6f]/70 to-transparent"></div>
        </div>

        {/* Brand Header */}
        <div className="relative z-10 flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-accent-orange to-amber-400 flex items-center justify-center text-white shadow-lg animate-bounce-slow">
        ...
        <div className="relative z-10 space-y-4 my-auto pt-6">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/15 backdrop-blur-md border border-white/25 text-xs font-bold font-poppins uppercase tracking-wider text-amber-300 shadow-sm">
            ★ Member Perks
          </div>
          
          <div key={authMode} className="animate-slide-up">
            <h2 className="font-serif text-[34px] font-bold leading-snug drop-shadow-sm text-white">
              {authMode === "signin"
                ...
                : "Join 50,000+ Visionary Global Travelers"}
            </h2>
            <p className="text-slate-100 text-[14px] leading-relaxed font-normal mt-3">
              {authMode === "signin"
                ? "Sign in to access personalized member rates, track active bookings, and save world destinations."
                : "Create an account today and instantly receive up to $250 off your first curated trip."}
            </p>
          </div>

          <ul className="space-y-3 pt-2 text-[13px] text-white font-medium">
            <li className="flex items-center gap-2.5 transition-transform hover:translate-x-1.5 duration-300 cursor-default">
      ...
      <div className="md:col-span-7 p-8 sm:p-12 flex flex-col justify-center bg-gradient-to-b from-[#f8fafc] via-[#f1f5f9] to-[#e8eef3] relative z-10">
        ...
        <div className="relative flex bg-slate-200/90 p-1.5 rounded-2xl mb-8 max-w-xs mx-auto md:mx-0 border border-slate-300/60 shadow-inner">
          <div
            className={`absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-white rounded-xl shadow-md border border-slate-200 transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
              authMode === "signup" ? "translate-x-[calc(100%+6px)]" : "translate-x-0"
            }`}
          />
          <button
            type="button"
            onClick={() => setAuthMode("signin")}
            className={`relative z-10 flex-1 py-2.5 px-4 rounded-xl font-poppins text-xs font-bold transition-all duration-300 ${
              authMode === "signin"
                ? "text-primary scale-105"
                : "text-slate-600 hover:text-primary scale-100"
            }`}
          >
            Sign In
          </button>
        ...
        {/* Form Heading */}
        <div key={authMode} className="mb-6 animate-slide-up">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <button
            ...
            className="flex items-center justify-center gap-2 py-3.5 px-4 rounded-2xl bg-white border border-slate-300/80 hover:border-accent-orange/50 hover:bg-slate-50 hover:shadow-md hover:-translate-y-0.5 active:scale-95 transition-all duration-200 text-slate-700 text-xs font-bold font-poppins"
          >
        ...
        <div className="relative flex items-center justify-center my-4">
          ...
          <span className="bg-[#f0f4f8] px-3 text-slate-500 text-[11px] font-bold uppercase tracking-wider font-poppins absolute">
            Or with email
          </span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            ...
            <div className="relative group">
              <User className="w-4 h-4 text-slate-400 absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:text-accent-orange transition-colors" />
              <input
                type="text"
          ...
          <div>
            ...
            <div className="relative group">
              <Mail className="w-4 h-4 text-slate-400 absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:text-accent-orange transition-colors" />
              <input
                type="email"
                required
                value={email}
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              ...
              {authMode === "signin" && (
                <a
                  href="#"
                  className="text-xs font-semibold text-accent-orange hover:underline font-poppins transition-colors"
                >
                  Forgot Password?
                </a>
              )}
            ...
            <div className="relative group">
              <Lock className="w-4 h-4 text-slate-400 absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:text-accent-orange transition-colors" />
              <input
          ...
          <button
            type="submit"
            className="w-full mt-2 group relative overflow-hidden bg-gradient-to-r from-accent-orange via-amber-500 to-accent-orange bg-[length:200%_auto] hover:bg-right text-white py-4 rounded-2xl font-poppins font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-accent-orange/30 hover:shadow-accent-orange/50 active:scale-[0.98] transition-all duration-500"
          >
